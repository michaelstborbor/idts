...
- **`Base.metadata.create_all()` instead of real migrations** — this
  bit us for real in production: `create_all()` only creates brand-new
  tables, it never adds a column to a table that already exists, so the
  live database from an earlier deploy didn't have `children.address` or
  the new `schedule_entries` columns when that code shipped, and every
  request touching those tables failed with `UndefinedColumn`. Fixed with
  a narrow, tested stop-gap (`app/db/schema_sync.py`, run on every
  startup): it compares each existing table's columns against the model
  and adds anything missing as a nullable column. This handles additive
  changes (new tables, new nullable columns) automatically going forward
  — it is still not a substitute for real migrations (Alembic) once a
  change is more than additive (renames, type changes, backfilling a
  NOT NULL column). That remains the honest next step, not this.

## Not yet built

- Alembic migrations (the schema-sync stop-gap above covers additive
  changes only — see the note above for exactly what it does and doesn't
  handle)
