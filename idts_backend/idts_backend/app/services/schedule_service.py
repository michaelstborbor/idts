...
def is_fully_immunized_child(db: Session, child: Child, evaluations: Optional[list[DoseEvaluation]] = None) -> bool:
    """
    "Fully Immunized Child" (FIC), per Ministry of Health protocol as
    specified by the programme: a child who has ACTUALLY RECEIVED every
    dose in the schedule up to and including Measles-Rubella dose 2 (MR2)
    — not merely "nothing currently outstanding for their age".

    This is a real definitional change from an earlier version of this
    dashboard stat, which measured "up to date for age" (nothing in
    due_soon/due/overdue/defaulter status) — under that older measure a
    newborn with nothing due yet was counted as "fully immunized", which
    does not match standard EPI/coverage-survey usage of the term. Under
    THIS definition, a child cannot be "fully immunized" until they are
    actually old enough to have received the full course AND have
    received it — a 2-month-old can never qualify, no matter how
    up-to-date they are for their age, because MR2 isn't due until 15
    months. That is intentional, not a bug: it matches how "Fully
    Immunized Child" coverage is actually defined and reported in EPI
    programmes.

    Cutoff rule: every schedule entry whose recommended_age_days is <=
    the MR2 entry's recommended_age_days must be status "administered".
    This is inclusive of any other antigen dose that happens to fall on
    the exact same recommended age as MR2 (in the seeded schedule,
    Malaria vaccine dose 4 also falls at 15 months) — treated as part of
    "up to MR2" because it is due at or before that same milestone, not
    after it. If the programme wants a narrower cutoff (e.g. explicitly
    excluding same-age doses that aren't MR2 itself), this is the one
    function to change — every caller (dashboard, per-child profile) goes
    through this, so there is exactly one place the rule lives.

    If no MR2 entry exists in the active schedule (e.g. a future schedule
    edit removes it), this returns False for every child rather than
    guessing at a different cutoff — a missing reference point should be
    visibly wrong, not silently produce a different result.
    """
    entries = get_active_schedule_entries(db)
    mr2 = next((e for e in entries if e.antigen.startswith("Measles-Rubella") and e.dose_number == 2), None)
    if mr2 is None:
        return False

    cutoff_days = mr2.recommended_age_days
    relevant_entry_ids = {str(e.id) for e in entries if e.recommended_age_days <= cutoff_days}
    if not relevant_entry_ids:
        return False
    ...
    administered_ids = {
        str(e.schedule_entry.id) for e in evaluations if e.status.value == "administered"
    }
    return relevant_entry_ids.issubset(administered_ids)
