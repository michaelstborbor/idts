...
# ============================================================
# Schema-sync safety net — fixes the real production bug where
# create_all() never adds columns to tables that already exist.
# ============================================================

def test_schema_sync_is_a_no_op_on_sqlite():
    """
    This is the one part of schema_sync.py that can actually be exercised
    in this sandbox (no real Postgres server available here) — confirming
    it correctly does nothing on SQLite rather than attempting Postgres-only
    SQL syntax (IF NOT EXISTS column-add) against a dialect that might not
    support it the same way. The Postgres path itself (the part that fixes
    the real "column children.address does not exist" production error)
    could not be tested against a live Postgres instance in this
    environment — see the conversation this was built in for the actual
    production log that confirmed it fixed the real issue after deploy.
    """
    from app.db.schema_sync import sync_missing_columns
    result = sync_missing_columns(TEST_ENGINE)
    assert result == []
