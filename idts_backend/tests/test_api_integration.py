...
def test_dashboard_stats_counts_given_and_fully_immunized(client, seeded):
    headers = auth_headers(client)

    # Child A: fully immunized for age — a newborn with nothing outstanding
    # yet (nothing is due at day 0 beyond what we haven't reached).
    child_a = client.post(
        "/api/v1/children",
    ...
    # Child B: overdue for BCG (100 days old, nothing given).
    client.post(
        "/api/v1/children",
    ...
    # Give child A their BCG so "given_total" has something to count.
    ...
    assert stats["fully_immunized"] == 1  # child A: BCG given, nothing else due yet
