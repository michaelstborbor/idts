...
class ScheduleEntry(Base, TimestampMixin):
    """
    One dose row — mirrors app.domain.schedule_engine.ScheduleEntry for the
    timing fields, plus clinical administration details (dosage/route/site)
    that are display-only and never touch the due/overdue calculation.
    All numeric timing values [CONFIG] unless sourced from an official
    schedule document (see seed.py for provenance notes).
    """
    ...
    # Clinical administration details, sourced from the official schedule
    # document — display-only, never used in due/overdue arithmetic.
