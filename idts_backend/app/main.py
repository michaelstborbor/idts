...
from app.api.routes import auth, children, defaulters, facilities, users, vaccinations
...
async def lifespan(app: FastAPI):
    ...
    Safe to run on every restart: Base.metadata.create_all() is a no-op for
    tables that already exist, and app.seed.seed() checks for existing users
    before inserting anything (see app/seed.py) — this will never duplicate
    data or overwrite something that's already there.

    NOTE: create_all() is fine for this MVP but is not a substitute for real
    schema migrations (Alembic) once the schema needs to evolve after real
    data exists — flagged in the backend README as not-yet-built.
    """
