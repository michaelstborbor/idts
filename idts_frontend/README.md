...
against the actual running backend — login, register a child, watch a
real defaulter status get computed server-side, record a vaccination,
assign a defaulter (adding a brand-new CHW inline, since no roster
exists yet), record a tracing attempt, and check the dashboard. Every
